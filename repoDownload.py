...
TOKEN='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'
...
def downloadThenUpload(sonar, parent_dir, github, item):
    ...
    if tagNameReq.status_code == 200 and tagNameReq.text.strip():
        tagNameList=tagNameReq.json()
    else:
        print("Empty response or error received from server")
        tagNameList=[]
    ...
    for tagQueue in tagNameList:
        ...
        directory=sanitize_dirname(repoToBeDownloaded['name']+tagName+'No'+str(tagcount))
        path = os.path.join(parent_dir, sanitize_dirname(directory))
        newPath=os.path.join(path,sanitize_dirname(directory))
...
def uploadToSonar(sonar, tagcount, tagName, directory, path):
    try:
        ...
        os.chdir(path)
        os.system(('mvn clean verify sonar:sonar -D maven.test.skip=true -D sonar.projectKey={projectKey} -D sonar.host.url=http://localhost:9000 -D sonar.login=squ_cfb9a12eff55b1f109a0cfa506f9c26feba690d1 -e -up -X').format(projectKey=directory))
